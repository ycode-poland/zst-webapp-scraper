...
pub async fn announcements(query: web::Query<QueryStr>) -> HttpResponse {
    let response = reqwest::get("http://www.zstrzeszow.pl/")
        .await
        .unwrap()
        .text()
        .await
    ...
    titles.zip(1..101).for_each(|(item, _i)| {
        ...
        announcements.push(Annoucement { title: text, date });
    });
